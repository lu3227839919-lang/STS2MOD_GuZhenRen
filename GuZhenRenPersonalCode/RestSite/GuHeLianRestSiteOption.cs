    ...
    : ModRestSiteOptionTemplate
{
    ...
    private static readonly LocString DescriptionText =
        new(
            "rest_site_ui",
            "OPTION_GU_ZHEN_REN_PERSONAL_HE_LIAN.description"
        );

    private static readonly LocString SelectionPrompt =
        new(
            "rest_site_ui",
            "OPTION_GU_ZHEN_REN_PERSONAL_HE_LIAN.selectionPrompt"
        );

    private static readonly LocString TargetSelectionPrompt =
        new(
            "rest_site_ui",
            "OPTION_GU_ZHEN_REN_PERSONAL_HE_LIAN.targetSelectionPrompt"
        );
    ...
    /// 合练篝火按钮图标。
    /// 将 PNG 文件放在：
    /// GuZhenRenPersonal/images/rest_site_options/GuHeLian.png
    ///
