...
public static class GuCardUsageRules
{
    ...
    {
        None, NotGuWorm, Sealed, UsesDepleted,
        InsufficientYuanQi, InsufficientImmortalEssence,
        SecondaryResourcePaymentBlocked,
    }

    public readonly record struct ActivationEligibility(
        bool IsAllowed,
    ...
    {
        internal static ActivationEligibility Allowed() =>
            new(true, ActivationFailure.None);
        internal static ActivationEligibility Rejected(
            ActivationFailure failure,
            int available = 0,
            int required = 0
        ) => new(false, failure, available, required);
        public string Describe() => Failure switch
        {
            ActivationFailure.NotGuWorm => "不是蛊虫牌",
            ActivationFailure.Sealed => "蛊虫处于封存状态",
            ActivationFailure.UsesDepleted => "本轮催动次数已用尽",
            ActivationFailure.InsufficientYuanQi =>
                $"元气不足（{Available}/{Required}）",
            ActivationFailure.InsufficientImmortalEssence =>
                $"仙元不足（{Available}/{Required}）",
            ActivationFailure.SecondaryResourcePaymentBlocked =>
                "次级资源支付条件不满足",
            _ => "可以催动",
        };
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(card);
        return GetActivationEligibility(card).IsAllowed;
    }

    /// <summary>无日志副作用的催动资格快照，供 UI 高频查询。</summary>
    public static ActivationEligibility GetActivationEligibility(CardModel card)
    {
        ArgumentNullException.ThrowIfNull(card);
        if (card is not IGuWormCard guCard)
            return ActivationEligibility.Rejected(ActivationFailure.NotGuWorm);
        if (GuSealSystem.IsSealed(card))
            return ActivationEligibility.Rejected(ActivationFailure.Sealed);
        if (GetRemainingUses(card, guCard) <= 0)
            return ActivationEligibility.Rejected(ActivationFailure.UsesDepleted);
        ...
        if (!IsBrilliantScarfFreePlay(card))
        {
            ...
            int available = SecondaryResourceCmd.Get(card.Owner, YuanQiSystem.ResourceId);
            if (available < cost)
                return ActivationEligibility.Rejected(
                    ActivationFailure.InsufficientYuanQi, available, cost
                );
        }

        int immortalCost = ImmortalEssenceSystem.GetActivationCost(guCard.GuRank);
        if (immortalCost > 0)
        {
            int available = ImmortalEssenceSystem.GetAvailableUnits(card.Owner);
            if (available < immortalCost)
                return ActivationEligibility.Rejected(
                    ActivationFailure.InsufficientImmortalEssence,
                    available,
                    immortalCost
        ...
        if (!HasPreparedActivationPayment(card) &&
            !CreateActivationPaymentPlan(card).IsAffordable)
            return ActivationEligibility.Rejected(
                ActivationFailure.SecondaryResourcePaymentBlocked
            );

        return ActivationEligibility.Allowed();
    }
