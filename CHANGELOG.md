...
### 修复
- 凡道杀招推演的元气改为整个推演流程成功收口后才实际扣除：取消、配方无效、资源不足、状态变化、创建失败或无法进入手牌等失败路径不再消耗元气。
- 血寄 `XueJiPower` 与万我临时生命 `WoLiTempHpPower` 不再显示在 Power 栏（`IsVisibleInternal => false`，寄生监听与结算逻辑不变）。

### 调整
- 万我临时生命血条：`WoLiTempHpPower` 实现 `IHealthBarVisualGraftSource`，剩余临时生命以蓝色额外血条显示在当前生命值右侧（RitsuLib 0.5.13 自动收集）。
- 万我生成我力虚影数量公式调整：由"不同兽力蛊种类数"改为"兽力蛊卡牌总张数"，公式为 `1 + 兽力虚影数量 + floor(max(0, 兽力蛊总数量 - 兽力虚影数量) / 2)`，卡牌描述与结算统一调用同一方法。
- 力道卡牌与 Power 卡图更新：万我、伤力回天、我力虚影、黑天大手印、苦力/群力/我力/自力更生/伤力回天 Power 图标。
- 精简光道、宙道、力道、血道及杀招的卡牌描述文案。

## [0.13.0] - 2026-08-18

### 新增
- 杀招推演费用重做：仙道杀招（材料含六转以上蛊虫）改为按转数消耗仙元（6/7/8/9 转分别扣 1/2/4/8 单位），不再消耗元气与推演能量；凡道杀招保持元气 + 1 能量；推演出的所有杀招首次打出不消耗能量。
- 推演材料扩充：杀招推演的材料可选自蛊手牌与蛊恢复牌堆（冷却中未封存的蛊虫）。
- 新增杀招：伤力回天（苦力蛊 + 自力更生蛊）、万我（力道杀招，材料永久封存）。
- 力道体系扩展：新增拔山、挽澜、我力蛊、群力蛊、苦力蛊、自力更生蛊；群力蛊同一张虚影卡可多次触发；我力虚影与兽力虚影界限统一。
- 伴生牌系统统一：力道与宙道的伴生牌生成/绑定合并到 Core 的 `CompanionCardSystem`，新增统一接口 `ICompanionSourceGuCard`/`ICompanionCard`，支持每张来源蛊一张（力道）与每种来源蛊一张（宙道）两种配对模式。

### 调整
- 元气回复数值调整：配合新元气曲线微调每回合元气回复。
- 卡框配色调整；拔山/挽澜/万我/我力/群力等新卡卡图。
- 删除蛊卡牌上限与替换界面：永久牌组不再限制蛊虫数量，移除满容量替换交互。

### 修复
- 杀招推演流程修复（含伤力回天触发、虚影联动等）。
...
### 修复
- 元气说明文案与实际机制不一致：元气悬停提示与资源描述仍显示 0.9.6 之前的旧曲线（上限 5/6/7/8/9/11/12/13/15、开场 4~14、回复 2~5），已同步为当前曲线（上限 5/5/6/6/7/7/8/8/9、开场 4~8、回复 2~4）；同时修正资源描述中"回合开始时清空"的错误说明。

## [0.9.11] - 2026-08-09

### 修复
- 修复多人 QuickSL 重载战斗后蛊存放牌堆可能不同步：开场五张蛊牌不再消费未进入原版同步快照的可变模组 RNG 流，改为由玩家网络 ID、楼层、稳定候选属性与 `NetCombatCard` ID 派生无状态确定性洗牌；同一战斗反复加载仍选择完全相同的卡牌与顺序。
- 开场日志增加所选蛊牌及其网络 ID，便于后续直接核对主客机自定义牌堆状态。
